...
export default class AImport extends Component {
  ...
  async populateImportData() {
    fetch("/api/import/all").then(response => response.json()).then(data => this.setState({ import: data }));
    fetch(`/api/import/alldetails`).then(response => response.json()).then(data => this.setState({ importDetail: data }));
  }
}