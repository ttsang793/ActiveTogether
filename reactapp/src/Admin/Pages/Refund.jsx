...
export default class ARefund extends Component {
  ...
  async populateRefundData() {
    fetch("/api/refund/get").then(reponse => reponse.json()).then(data => {
      this.setState({ loading: false, refunds: data }, () => console.log(this.state.refunds))
    })
  }
      ...
      <tr key={i}>
        ...
        <td className='align-middle'>
          <select defaultValue={r.status} onChange={e => this.handleStatus(e, i)}>
            ...
            <option value="0" disabled>Yêu cầu</option>
  ...
  async updateStatus(e, refund) {
    ...
    if (confirm("Bạn có chắc chắn hoàn thành xử lý trả hàng cho khách?")) {
      ...
      if (refund.status == 1) {
        if (confirm("Bạn có muốn cập nhật số lượng sản phẩm?")) updateQuantity = 1;
      }
      
      const response = await fetch(`/api/refund/update?updateQuantity=${updateQuantity}`, {
        method: "POST",
        headers: {
