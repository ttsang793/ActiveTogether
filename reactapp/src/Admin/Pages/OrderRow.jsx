                ...
                <tbody className="table-group-divider">
                  {orderDetail.map(o =>
                    <tr key={o.sku}>
                      ...
                      <td className="align-middle">{o.color + " " + o.size}</td>
      ...
      <tr>
        ...
        <td className="align-middle">
          <select value={status} onChange={handleStatusChange}>
            <option value="0" disabled>Đã đặt hàng</option>
            <option value="1" disabled={status >= 1 || status === -1 }>Đã xác nhận</option>
            <option value="2" disabled={status >= 2 || status === -1 }>Đã giao hàng</option>
            <option value="3" disabled>Đã nhận hàng</option>
            <option value="-1" disabled>Đã hủy</option>
          </select>
        </td>
        <td className="align-middle">{DisplayPrice(props.order.total)}</td>
        <td>
          <button className="small-at-btn me-1" onClick={() => updateOrder(props.order.id, status)}>Lưu</button>
